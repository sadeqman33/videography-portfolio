...
export default function Hero() {
    const { heroConfig, videos, customMediaUrls } = useSiteData();

    // Helper to resolve video and thumbnail for 3D showcase cards
    const resolveCardMedia = (slugOrId, defaultSlug) => {
        const target = slugOrId || defaultSlug;
        const found = videos?.find(v => v.slug === target || v.id === target || v.id === Number(target));
        if (found) {
            const previewSrc = (found.videoBlobKey && customMediaUrls[found.videoBlobKey])
                || found.previewUrl
                || found.videoUrl
                || (found.slug ? `/videos/${found.slug}/preview.mp4` : `/videos/${defaultSlug}/preview.mp4`);

            const thumbSrc = (found.thumbBlobKey && customMediaUrls[found.thumbBlobKey])
                || found.thumbnailUrl
                || (found.slug ? `/videos/${found.slug}/thumbnail.jpg` : `/videos/${defaultSlug}/thumbnail.jpg`);

            return { title: found.title, previewSrc, thumbSrc };
        }

        return {
            title: '',
            previewSrc: `/videos/${target}/preview.mp4`,
            thumbSrc: `/videos/${target}/thumbnail.jpg`
        };
    };

    const leftCard = resolveCardMedia(heroConfig?.leftVideoSlug, 'charger_superbee');
    const centerCard = resolveCardMedia(heroConfig?.centerVideoSlug, 'cadillac');
    const rightCard = resolveCardMedia(heroConfig?.rightVideoSlug, 'nissan_patrol');
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
                    ...
                    <div className="animate-fade-in-up text-right">
                        <span className="inline-block px-4 py-1.5 bg-white/5 border border-white/10 rounded-full text-xs font-bold text-neutral-300 tracking-wider mb-6">
                            {heroConfig?.badge || "مصور فيديو وصانع محتوى إعلاني وسينمائي"}
                        </span>
                        <h1 className="text-4xl lg:text-6xl font-black tracking-tight text-white mb-6 leading-[1.2]">
                            {heroConfig?.titleLine1 || "فيديوهات سينمائية"}{' '}
                            <span className="text-silver-gradient block mt-3">
                                {heroConfig?.titleLine2 || "تزيد مبيعاتك وتلفت الأنظار"}
                            </span>
                        </h1>
                        <p className="text-lg lg:text-xl text-neutral-400 mb-10 max-w-lg delay-100 animate-fade-in-up opacity-0 leading-relaxed" style={{ animationFillMode: 'forwards' }}>
                            {heroConfig?.description || "نصنع فيديوهات ترويجية وإعلانات سينمائية مبتكرة تجذب جمهورك المستهدف في العراق وتساهم في مضاعفة أرباح عملك."}
                        </p>
                        <div className="flex flex-col sm:flex-row gap-5 delay-200 animate-fade-in-up opacity-0" style={{ animationFillMode: 'forwards' }}>
                            <a
                                href={heroConfig?.primaryBtnLink || "#booking"}
                                className="inline-flex items-center justify-center px-8 py-4 text-base font-bold text-black bg-white hover:bg-neutral-200 rounded-full transition-all duration-300 shadow-[0_0_20px_rgba(255,255,255,0.15)] hover:shadow-[0_0_30px_rgba(255,255,255,0.35)] hover:-translate-y-1 cursor-pointer"
                            >
                                {heroConfig?.primaryBtnText || "احجز جلستك الآن"}
                                <ArrowLeft className="mr-2 w-5 h-5" />
                            </a>
                            <a
                                href={heroConfig?.secondaryBtnLink || "#portfolio"}
                                className="inline-flex items-center justify-center px-8 py-4 text-base font-medium text-white bg-white/5 hover:bg-white/10 rounded-full backdrop-blur-sm border border-white/10 hover:border-white/20 transition-all duration-300 cursor-pointer"
                            >
                                <Play className="ml-2 w-5 h-5 text-neutral-300" />
                                {heroConfig?.secondaryBtnText || "شاهد أعمالي"}
                            </a>
                        </div>
                    </div>
                    ...
                    <div className="relative delay-300 animate-fade-in-up opacity-0 w-full h-[520px] flex items-center justify-center [perspective:1200px] lg:ml-auto" style={{ animationFillMode: 'forwards' }}>
                        <div className="relative w-full max-w-[340px] h-[480px] group cursor-pointer flex items-center justify-center">
                            ...
                            <div className="absolute w-[180px] aspect-[9/16] rounded-2xl overflow-hidden border border-white/10 bg-[#0c0a09] transition-all duration-700 transform -rotate-12 -translate-x-20 translate-y-6 scale-90 opacity-40 group-hover:-translate-x-28 group-hover:-rotate-[18deg] group-hover:opacity-70 group-hover:scale-95 shadow-xl origin-bottom">
                                <video
                                    key={leftCard.previewSrc}
                                    src={leftCard.previewSrc}
                                    poster={leftCard.thumbSrc}
                                    autoPlay
                                    muted
                                    loop
                                    playsInline
                            ...
                            <div className="absolute w-[180px] aspect-[9/16] rounded-2xl overflow-hidden border border-white/10 bg-[#0c0a09] transition-all duration-700 transform rotate-12 translate-x-20 translate-y-6 scale-90 opacity-40 group-hover:translate-x-28 group-hover:rotate-[18deg] group-hover:opacity-70 group-hover:scale-95 shadow-xl origin-bottom">
                                <video
                                    key={rightCard.previewSrc}
                                    src={rightCard.previewSrc}
                                    poster={rightCard.thumbSrc}
                                    autoPlay
                                    muted
                                    loop
                                    playsInline
                            ...
                            {/* Card 2 (Center Video - Main Featured) */}
                            <div className="absolute w-[200px] aspect-[9/16] rounded-3xl overflow-hidden border border-white/15 bg-black transition-all duration-700 transform rotate-0 scale-100 z-10 shadow-[0_0_30px_rgba(255,255,255,0.07)] group-hover:scale-105 group-hover:-translate-y-4 group-hover:shadow-silver-glow group-hover:border-white/20 origin-bottom">
                                <video
                                    key={centerCard.previewSrc}
                                    src={centerCard.previewSrc}
                                    poster={centerCard.thumbSrc}
                                    autoPlay
                                    muted
                                    loop
                                    playsInline
                                ...
                                <div className="absolute bottom-5 left-3 right-3 bg-black/70 backdrop-blur-md border border-white/10 rounded-xl p-3 flex items-center justify-between transition-transform duration-700 translate-y-2 group-hover:translate-y-0" dir="rtl">
                                    <div className="text-right">
                                        <p className="text-white font-bold text-[11px]">
                                            {heroConfig?.centerBadgeTitle || "أحدث الإعلانات"}
                                        </p>
                                        <p className="text-neutral-400 text-[9px] mt-0.5">
                                            {heroConfig?.centerBadgeSubtitle || (centerCard.title || "عمل سينمائي مميز")}
                                        </p>
                                    </div>
                                    <span className="bg-white/15 text-white px-2 py-0.5 rounded-full text-[8px] font-bold border border-white/10">
                                        {heroConfig?.centerBadgeQuality || "4K Ultra HD"}
                                    </span>
                                </div>
